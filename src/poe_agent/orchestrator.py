...
_DELEGATION_STATUS: dict[str, str] = {
...
def _tool_status_label(name: str, tool_input: dict) -> str:
    ...
    return _STATUS_LABELS.get(name, f"Using {name}")
...
class Orchestrator:
    ...
    def run(
        ...
        while decision.type != "answer":
            ...
            if "tools" in inp:
                # Check if any tool calls are delegation tools
                delegation_calls = [
                    tc for tc in inp["tools"] if tc["name"] in DELEGATION_TOOL_NAMES
                ]
                regular_calls = [
                    tc for tc in inp["tools"] if tc["name"] not in DELEGATION_TOOL_NAMES
                ]

                results = []

                # Handle delegation tool calls
                for tool_call in delegation_calls:
                    if on_status:
                        on_status(
                            _DELEGATION_STATUS.get(
                                tool_call["name"], f"Delegating {tool_call['name']}"
                            )
                        )
                    logger.info("DELEGATION %s input=%s", tool_call["name"], tool_call["input"])
                    ...
                    self._accumulated_research.append(
                        f"[{tool_call['name']}] {delegation_result[:2000]}"
                    )
                ...
                for tool_call in regular_calls:
                    if on_status:
                        on_status(
                            _tool_status_label(tool_call["name"], tool_call["input"])
                        )
    ...
    def force_answer(self, extra_context: str = "") -> str:
        ...
        # Reset answerer for a clean call
    ...
    def _run_agent_to_completion(self, agent_name: str, task: str) -> str:
        ...
        This is what makes delegation work without modifying agent.py.
        """
        ...
        # Loop until the sub-agent produces an answer or routes to its next agent
        while decision.type != "answer":
            inp = decision.input

            if "tools" in inp:
                # Regular tool execution within the sub-agent
                results = []
                for tool_call in inp["tools"]:
                    ...
                    logger.info("DELEGATION_TOOL %s input=%s", tool_call["name"], tool_call["input"])
                    tool_result = self.steps[tool_call["name"]].call(tool_call["input"])
                    result_content = (
                        json.dumps(tool_result.input["result"])
                        if isinstance(tool_result.input["result"], (dict, list))
                    ...
                    logger.info("DELEGATION_TOOL_RESULT %s (%d chars)", tool_call["name"], len(result_content))
                    results.append(
                        {
                            "tool_use_id": tool_call["id"],
                            "content": result_content,
                    ...
                    # Check budget — return partial results if exceeded
                    if self.api_calls >= self.max_api_calls:
                        logger.warning(
                            "Budget exceeded during delegation to %s, returning partial results",
                            agent_name,
                        )
                ...
                if self._on_status:
                    self._on_status("Analyzing results...")
                decision = self._call_agent(inp["return_to"], {"tool_results": results})

            elif "target" in inp:
                # Sub-agent wants to route to its next agent (e.g. researcher → answerer).
                # Intercept: capture the query as the output instead of routing.
                logger.info(
                    "DELEGATION_INTERCEPT %s -> %s (capturing output)",
                    agent_name,
                    inp["target"],
                )
                return inp["query"]

        # Sub-agent produced a direct answer
    ...
    def _status_label(self, decision: NextStep) -> str:
        ...
        if "tools" in inp and inp["tools"]:
            ...
            if first_tool["name"] in DELEGATION_TOOL_NAMES:
                return _DELEGATION_STATUS.get(first_tool["name"], "Delegating...")
